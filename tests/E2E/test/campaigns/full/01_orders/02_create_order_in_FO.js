...
scenario('Create order in the Front Office', () => {
  scenario('Open the browser and connect to the Front Office', client => {
    test('should open the browser', async () => {
      await client.open();
      await client.startTracing('create_order_in_FO_1');
    });
...
scenario('Check the created order in the Back Office', () => {
  scenario('Open the browser and connect to the Back Office', client => {
    test('should open the browser', async () => {
      await client.open();
      await client.startTracing('create_order_in_FO_2');
    });
  ...
  scenario('Get the customer information', client => {
    ...
    test('should search for the customer email in the "Customers list"', async () => {
      await client.isVisible(Customer.customer_filter_by_email_input, 2000);
      if (global.isVisible) {
        await client.search(Customer.customer_filter_by_email_input, 'pub@prestashop.com');
      }
    });
    test('should click on "View" button', async () => {
      await client.scrollWaitForExistAndClick(Customer.dropdown_toggle, 50, 2000);
      await client.waitForExistAndClick(Customer.view_button, 1000);
    });
...
scenario('Check that Ordering more than the stock is giving a wrong message', () => {
  ...
  scenario('Order a product from the quick view', client => {
    ...
    test('should click on "Quick view" button', () => {
      return promise
        .then(() => client.moveToObject(SearchProductPage.product_result_name, 3000))

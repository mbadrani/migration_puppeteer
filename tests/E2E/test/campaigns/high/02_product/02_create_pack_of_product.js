...
scenario('Create a pack of products in the Back Office', client => {
  ...
  scenario('Edit the Basic settings', client => {
    test('should set the "product name"', () => client.fillInputText(AddProductPage.product_name_input, data.pack.name + date_time));
    ...
    test('should set the "Add products to your pack"', () => {
      return promise
        .then(() => client.fillInputText(AddProductPage.search_product_pack, data.pack.pack.pack1.search))
        .then(() => page.waitForSelector(AddProductPage.product_item_pack))
        .then(() => page.$eval(AddProductPage.product_item_pack, elem => elem.click()))
        .then(() => client.fillInputNumber(AddProductPage.product_pack_item_quantity, data.pack.pack.pack1.quantity))
        .then(() => page.$eval(AddProductPage.product_pack_add_button, elem => elem.click()));
    });
    test('should set the "Add products to your pack"', () => {
      return promise
        .then(() => client.fillInputText(AddProductPage.search_product_pack, data.pack.pack.pack2.search))
        .then(() => page.waitForSelector(AddProductPage.product_item_pack))
        .then(() => page.$eval(AddProductPage.product_item_pack, elem => elem.click()))
        .then(() => client.fillInputNumber(AddProductPage.product_pack_item_quantity, data.pack.pack.pack2.quantity))
        .then(() => page.$eval(AddProductPage.product_pack_add_button, elem => elem.click()));
    });
    ...
    test('should set the "New category name"', () => client.fillInputText(AddProductPage.product_category_name_input, data.pack.new_category_name + date_time));
...
scenario('Check the product creation in the Back Office', client => {test('should open browser', () => client.open());
  test('should log in successfully in BO', () => client.signInBO(AccessPageBO));
  test('should go to "Catalog" page', () => client.goToLink(Menu.Sell.Catalog.products_submenu));
...
scenario('Check the pack product in the Front Office', () => {
  ...
  scenario('Check that the pack product is well displayed in the Front Office', client => {
    ...
    test('should check that the first product pack name is equal to "The adventure begins Framed poster Dimension-40x60cm"', () => client.checkTextValue(productPage.pack_product_name.replace('%P', 2), 'The adventure begins Framed poster Dimension-40x60cm'));
    test('should check that the first product pack price is equal to "€34.80"', () => client.checkTextValue(productPage.pack_product_price.replace('%P', 2), '€34.80'));
    test('should check that the first product pack quantity is equal to "1"', () => client.checkTextValue(productPage.pack_product_quantity.replace('%P', 2), 'x 1'));
    test('should check that the second product pack name is equal to "Today is a good day Framed poster Dimension-40x60cm"', () => client.checkTextValue(productPage.pack_product_name.replace('%P', 3), 'Today is a good day Framed poster Dimension-40x60cm'));
    test('should check that the second product pack price is equal to "€34.80"', () => client.checkTextValue(productPage.pack_product_price.replace('%P', 3), '€34.80'));
    test('should check that the second product pack quantity is equal to "3"', () => client.checkTextValue(productPage.pack_product_quantity.replace('%P', 3), 'x 3'));
